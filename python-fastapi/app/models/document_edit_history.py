"""文档编辑历史模型"""
from sqlalchemy import Column, Integer, String, ForeignKey, DateTime, Text, func, UniqueConstraint, Index
...
class DocumentEditHistory(Base):
    ...
    __table_args__ = (
        UniqueConstraint('document_id', 'edited_user_id', name='uniq_document_id_edited_user_id'),
        # Index('idx_document_id', 'document_id'),
        # Index('idx_edited_user_id', 'edited_user_id'),
    )

    id = Column[str](String(36), primary_key=True, default=lambda: str(uuid4()))
    document_id = Column[str](String(36), ForeignKey("document_base.id"), index=True, nullable=False, comment="所属文档")
    edited_user_id = Column[int](Integer, ForeignKey("user.id"), index=True, nullable=False, comment="编辑的用户")
